...
class MultimodalProcessorTest {
    ...
    fun `test weighted fusion strategy`() {
        ...
        val textResult = fusedResult["text"]
        val imageResult = fusedResult["image"]

        assertNotNull(textResult)
        assertNotNull(imageResult)

        assertTrue(textResult.toString().contains("weight"))
        assertTrue(imageResult.toString().contains("weight"))
    }
    ...
    fun `test with context`() {
        ...
        val contextResult = fusedResult["context"]?.jsonObject
        assertNotNull(contextResult)
        assertEquals(context.id, contextResult!!["id"]?.toString()?.replace("\"", ""))
    }
}
