...
class SemanticDocumentSplitter(
     ...
     * @return 合并后的片段列表
     */
    private fun mergeSegmentsBySimilarity(segmentEmbeddings: List<Pair<String, FloatArray>>): List<String> {
        ...
        for (i in 1 until segmentEmbeddings.size) {
            ...
            // 计算当前片段和下一个片段之间的相似度
            val similarity = ai.kastrax.rag.util.cosineSimilarity(currentEmbedding, nextEmbedding)
