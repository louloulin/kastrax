---
title: "Retrieval, Semantic Search, Reranking | RAG | Kastrax Docs"
description: Guide on retrieval processes in Kastrax's RAG systems, including semantic search, filtering, and re-ranking.
---
...
Kastrax provides flexible retrieval options with support for semantic search, filtering, and re-ranking.
...
Filter results based on metadata fields to narrow down the search space. This is useful when you have documents from different sources, time periods, or with specific attributes. Kastrax provides a unified MongoDB-style query syntax that works across all supported vector stores.
