---
title: "Using MCP With Kastrax | Agents | Kastrax Docs"
description: "Use MCP in Kastrax to integrate third party tools and resources in your AI agents."
---

# Using MCP With Kastrax ✅
...
MCP in Kastrax provides a standardized way to connect to tool servers. It supports Stdio and HTTP (uses Streamable HTTP, falls back to SSE).
...
```bash
pnpm add @kastrax/mcp@latest
```

Using npm:

```bash copy
npm install @kastrax/mcp@latest
```

## Using MCP in Your Code ✅

The `MCPClient` class provides a way to manage multiple tool servers in your Kastrax applications without managing multiple MCP clients. The `KastraxMCPClient` used internally by `MCPConfiguration` now automatically detects the transport type based on your server configuration:
...
const response = await agent.stream(
  "What's new in Kastrax and how's the weather?",
  {
    toolsets,
  },
);
...
We've curated an [MCP Registry Registry](https://kastrax.ai/mcp-registry-registry) to help you find the best places to source MCP servers, but here's how you can use tools from some of our favorites:
...
[Composio.dev](https://composio.dev) provides a registry of [SSE-based MCP servers](https://mcp.composio.dev) that can be easily integrated with Kastrax. The SSE URL that's generated for Cursor is compatible with Kastrax - you can use it directly in your configuration:
...
[Smithery.ai](https://smithery.ai) provides a registry of MCP servers that you can easily use with Kastrax:
...
Looking to use Kastrax's MCP documentation server in your IDE? Check out our [MCP Documentation Server guide](/docs/getting-started/mcp-docs-server) to get started.
