---
title: "Handling Complex LLM Operations | Workflows | Kastrax"
description: "Workflows in Kastrax help you orchestrate complex sequences of operations with features like branching, parallel execution, resource suspension, and more."
---

# Handling Complex LLM Operations with Workflows

Workflows in Kastrax help you orchestrate complex sequences of operations with features like branching, parallel execution, resource suspension, and more.
...
Here's how you define a workflow in Kastrax. The `name` field determines the workflow's API endpoint (`/workflows/$NAME/`), while the `triggerSchema` defines the structure of the workflow's trigger data:

```ts filename="src/kastrax/workflow/index.ts"
...
Register your workflow with Kastrax to enable logging and telemetry:

```ts showLineNumbers filename="src/kastrax/index.ts"
...
The workflow can also have the kastrax instance injected into the context in the case where you need to create dynamic workflows:
...
```ts showLineNumbers filename="src/kastrax/run-workflow.ts" copy
...
Or use the API (requires running `kastrax dev`):
...
When you need to pause execution for external data, user input, or asynchronous events, Kastrax [supports suspension at any step](./suspend-and-resume.mdx), persisting the state of the workflow so you can resume it later.

## Observability and Debugging

Kastrax workflows automatically [log the input and output of each step within a workflow run](../../reference/observability/otel-config.mdx), allowing you to send this data to your preferred logging, telemetry, or observability tools.
