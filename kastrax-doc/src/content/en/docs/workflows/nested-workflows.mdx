...
fun main() = runBlocking {
    ...
    println("Nested workflow execution time: ${metrics.stepExecutionTimes["process-data"]} ms")
}
```
...
val options = WorkflowExecuteOptions(
    ...
    onStepFinish = { stepResult ->
        ...
        if (stepResult.nestedSteps != null) {
            println("Nested workflow ${stepResult.stepId} completed in $duration ms")
            println("Nested steps: ${stepResult.nestedSteps.size}")
        }
    },
...
fun main() = runBlocking {
    ...
    if (result.status == "suspended") {
        ...
        if (resumeResult.success) {
            ...
            println("Title: ${resumeResult.output["title"]}")
            println("Published URL: ${resumeResult.output["publishedUrl"]}")
        } else {
