---
title: "Branching, Merging, Conditions | Workflows | Kastrax Docs"
description: "Control flow in Kastrax workflows allows you to manage branching, merging, and conditions to construct workflows that meet your logic requirements."
---
...
Sometimes you need a step to execute only after multiple other steps have completed. Kastrax provides a compound `.after([])` syntax that allows you to specify multiple dependencies for a step.
...
Workflows often need to repeat steps until certain conditions are met. Kastrax provides two powerful methods for creating loops: `until` and `while`. These methods offer an intuitive way to implement repetitive tasks.
