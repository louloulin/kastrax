---
title: "Dynamic Workflows | Kastrax Docs"
...
The key to creating dynamic workflows is accessing the Kastrax instance from within a step's `execute` function and using it to create and run a new workflow.

### Basic Example

```typescript
import { Kastrax, Step, Workflow } from '@kastrax/core';
import { z } from 'zod';

const isKastrax = (kastrax: any): kastrax is Kastrax => {
  return kastrax && typeof kastrax === 'object' && kastrax instanceof Kastrax;
};
...
const createDynamicWorkflow = new Step({
  ...
  execute: async ({ context, kastrax }) => {
    if (!kastrax) {
      throw new Error('Kastrax instance not available');
    }

    if (!isKastrax(kastrax)) {
      throw new Error('Invalid Kastrax instance');
    }
    ...
    const dynamicWorkflow = new Workflow({
      ...
      kastrax, // Pass the kastrax instance to the new workflow
...
// Register the workflow with Kastrax
...
1. **Kastrax Instance**: The `kastrax` parameter in the `execute` function provides access to the Kastrax instance, which is essential for creating dynamic workflows.

2. **Error Handling**: Always check if the Kastrax instance is available before attempting to create a dynamic workflow.
...
4. **Workflow Lifecycle**: Dynamic workflows are not automatically registered with the main Kastrax instance. They exist only for the duration of the step execution unless you explicitly register them.
...
Dynamic workflows provide a powerful way to create flexible, adaptable workflow systems. By leveraging the Kastrax instance within step execution, you can create workflows that respond to runtime conditions and requirements.
