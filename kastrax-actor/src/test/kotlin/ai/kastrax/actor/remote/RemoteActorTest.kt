...
class RemoteActorTest {
    ...
    fun `test remote actor configuration`() {
        ...
        // 验证配置转换正确
        assertEquals("127.0.0.1", kactorConfig.host)
        assertEquals(8091, kactorConfig.port)
        assertEquals("test-host", kactorConfig.advertiseHostname)
        assertEquals(8092, kactorConfig.advertisePort)
    }
