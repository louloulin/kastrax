...
本文档详细介绍了 KastraX 框架中的 Anthropic 集成功能，包括基本用法、流式响应处理和工具调用支持。

## 1. 概述

Anthropic 集成是 KastraX 框架的一部分，允许开发者使用 Anthropic 的 Claude 系列大语言模型进行文本生成、流式响应和工具调用。该集成提供了以下核心功能：

- Anthropic Claude 模型支持（包括 Claude 3 Opus、Sonnet、Haiku 等）
- 流式响应处理
- 工具调用支持

## 2. 基本用法

### 2.1 创建 Anthropic 提供商

可以通过两种方式创建 Anthropic 提供商：
...
Anthropic 集成提供了两种流式响应处理方式：通过代理层和直接使用 AnthropicStreamingClient。
...
### 3.2 直接使用 AnthropicStreamingClient

```kotlin
// 创建流式客户端
val client = AnthropicStreamingClient("https://api.anthropic.com/v1", "your-api-key")
...
Anthropic 集成支持工具调用，允许模型使用预定义的工具执行特定任务。
...
可以为 Anthropic 客户端设置超时时间，以处理长时间运行的请求：

```kotlin
val client = AnthropicStreamingClient(
...
在使用 Anthropic 集成时，应该始终包含适当的错误处理：
...
- `AnthropicStreamingExample.kt`：展示如何通过代理层使用流式响应
- `AnthropicDirectStreamingExample.kt`：展示如何直接使用 AnthropicStreamingClient
...
2. **请求超时**：对于复杂的请求，可能需要增加超时时间：
   ```kotlin
   val client = AnthropicStreamingClient(baseUrl, apiKey, timeout = 120000)
   ```
...
2. 使用 `AnthropicStreamChunk` 的不同类型来跟踪流式响应的状态：
   ```kotlin
   when (chunk) {
       is AnthropicStreamChunk.Content -> { /* 处理内容 */ }
       is AnthropicStreamChunk.ToolUse -> { /* 处理工具使用 */ }
       is AnthropicStreamChunk.Finished -> { /* 处理完成 */ }
       is AnthropicStreamChunk.Done -> { /* 处理结束 */ }
   }
   ```
