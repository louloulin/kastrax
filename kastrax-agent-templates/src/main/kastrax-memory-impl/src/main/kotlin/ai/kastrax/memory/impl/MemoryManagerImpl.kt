...
class MemoryManagerImpl(
    ...
    override suspend fun queryMessages(threadId: String, options: MemoryQueryOptions): List<MemoryMessage> {
        ...
        return allMessages.filter { message ->
                ...
                when (filter) {
                    ...
                    is ContentFilter -> {
                        ...
                        matches = when (filter.matchType) {
                            ContentMatchType.CONTAINS -> content.contains(filter.query, ignoreCase = true)
                            ContentMatchType.EXACT -> content.equals(filter.query, ignoreCase = true)
                            ContentMatchType.STARTS_WITH -> content.startsWith(filter.query, ignoreCase = true)
                            ContentMatchType.ENDS_WITH -> content.endsWith(filter.query, ignoreCase = true)
                            ...
                            matches = when (filter.matchType) {
                                ...
                                MetadataMatchType.CONTAINS -> {
                                    ...
                                    if (value is String && filter.value is String) {
                                        value.contains(filter.value as String, ignoreCase = true)
                                    } else {
                                        value == filter.value
                                    }
                                }
            ...
            val sorted = when (options.sortBy) {
                ...
                SortField.PRIORITY -> {
                    if (options.sortDirection == SortDirection.ASC) {
                        filteredMessages.sortedBy { it.priority?.value ?: 0f }
                    } else {
                        filteredMessages.sortedByDescending { it.priority?.value ?: 0f }
                    }
                }
            ...
            // 应用处理器
            if (options.processors != null && options.processors.isNotEmpty()) {
    ...
    override suspend fun getMessageContext(selector: ContextSelector): List<MemoryMessage> {
        // 获取中心消息
        val centerMessage = storage.getMessages(selector.messageId, 1).firstOrNull()
            ?: throw IllegalArgumentException("Message not found: ${selector.messageId}")
        
        // 获取线程中的所有消息
        val allMessages = storage.getMessages(centerMessage.threadId, Int.MAX_VALUE)
    ...
    override suspend fun queryThreads(options: ThreadQueryOptions): List<MemoryThread> {
        ...
        return allThreads.filter { thread ->
            ...
            // 应用标题过滤
            if (options.titleFilter != null && thread.title != null) {
                if (!thread.title.contains(options.titleFilter, ignoreCase = true)) {
                    matches = false
                }
            }
            ...
            if (matches && options.metadataFilters.isNotEmpty() && thread.metadata != null) {
                for (filter in options.metadataFilters) {
                    matches = when (filter.matchType) {
                        MetadataMatchType.EQUALS -> thread.metadata[filter.key] == filter.value
                        MetadataMatchType.CONTAINS -> {
                            val value = thread.metadata[filter.key]
                            if (value is String && filter.value is String) {
                                value.contains(filter.value as String, ignoreCase = true)
                            } else {
                                value == filter.value
                        ...
                        MetadataMatchType.EXISTS -> thread.metadata.containsKey(filter.key)
                        MetadataMatchType.NOT_EXISTS -> !thread.metadata.containsKey(filter.key)
                    }
            ...
            val sorted = when (options.sortBy) {
                ...
                ThreadSortField.MESSAGE_COUNT -> {
                    ...
                    val threadMessageCounts = filteredThreads.associateWith { thread ->
                        storage.getMessages(thread.id, 1).size
                    }
    ...
    override suspend fun batchUpdateMessages(options: MessageBatchOptions, updates: MessageUpdateOptions): Int {
        ...
        val messages = when {
            ...
            options.threadId != null -> {
                ...
                queryMessages(options.threadId, queryOptions)
            }
            else -> {
        ...
        for (message in messages) {
            ...
            if (updates.priority != null) {
                if (storage.updateMessagePriority(message.id, updates.priority)) {
                    updated = true
                }
            }
            ...
            if (updates.metadata != null) {
                val newMetadata = message.metadata?.toMutableMap() ?: mutableMapOf()
                newMetadata.putAll(updates.metadata)
                
                // 这里假设存储实现有更新元数据的方法
                // 如果没有，需要添加这个方法
                if (storage is AdvancedMemoryStorage) {
                    if (storage.updateMessageMetadata(message.id, newMetadata)) {
                        updated = true
                    }
                }
            }
    ...
    override suspend fun batchDeleteMessages(options: MessageBatchOptions): Int {
        ...
        val messageIds = when {
            ...
            options.threadId != null -> {
                ...
                queryMessages(options.threadId, queryOptions).map { it.id }
            }
            else -> {
                throw IllegalArgumentException("Either messageIds or threadId must be provided")
            }
    ...
    override suspend fun getThreadStats(): ThreadStats {
        ...
        val oneWeekAgo = now.minus(DateTimePeriod(days = 7))
                ...
                buildString {
                    ...
                    messages.forEach { message ->
                        ...
                        if (message.priority != null) {
                            appendLine("Priority: ${message.priority.value}")
                        }
                        appendLine("---")
                    }
                }
    ...
    override suspend fun importThread(data: String, format: ExportFormat): String {
        return when (format) {
            ExportFormat.JSON -> {
                try {
                    // 解析JSON
                    val export = json.decodeFromString<ThreadExport>(data)
                    
                    // 创建新线程
                    val newThread = export.thread.copy(
